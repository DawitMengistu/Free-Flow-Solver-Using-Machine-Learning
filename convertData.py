...
folderNames = ["regular_pack", "green_pack", "bonus_pack", "blue_pack"]

for a in range (4):
    for f in range(30):
        # Load an image from file
        image_path = 'data/'+ str(folderNames[a]) +'/66/' + str(30 + f) + '.jpg'
        ...
        np_array1 = np.array(arr)
        np_array2 = np.array(updated_matrix)

        # Save the NumPy array to a file using np.save
        np.save('ds/a-66-' + str((a * 30) + 150 + f) + '.npy', np_array1)
        np.save('ds/g-66-' + str((a * 30) + 150 + f) + '.npy', np_array2)
        print("Done!", str((a * 30) + 150 + f))
